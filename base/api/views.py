...
def add_user_email(request):
    if request.method == "POST":
        ...
        email_host_use_tls = (
            True if request.data["email_host_use_tls"] == "on" else False
        )
...
def delete_user_email(request, user_id, email_id):
    ...
    if deletable_emails:
        emails_to_delete = ScheduledEmail.objects.filter(
            from_email__in=deletable_emails
        )
        if emails_to_delete:
...
def create_new_campaign(request, user_id):
    if request.method == "POST":
        ...
        if (
            not user_id
            or not name
            or not base_time_gap
            or not random_time_gap
            or not from_emails_ids
            or not to_emails
            or not subject
            or not body
        ):
        ...
        try:
            ...
            return Response(
                {"status": 400, "details": "Invalid format for 'from_emails'."}
            )
...
def run_campaign(request, user_id, campaign_id, action):
    status = None
    try:
        runnable_campaign = Campaign.objects.filter(user_id=user_id, id=campaign_id)
        if runnable_campaign:
            runnable_campaign = runnable_campaign.first()
            runnable_campaign.status = "In Progress" if action == "start" else "Stop"
            runnable_campaign.save()
            status = 200
    except:
        status = 400
